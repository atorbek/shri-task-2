## Тестовое задание «Напишите линтер» для [16-й Школы разработки интерфейсов](https://yandex.ru/promo/academy/shri) (зима 2020, Москва)

### При реализации использовал

`json-to-ast` инструмент для преобразования `json` к `ast` (абстрактное синтаксическое дерево). 
В проекте использовал для получения поля `loc`, которое содержит информацию о положении блока в исходной строке.
Инструмент дал возможность сэкономить время на создании утилитарных методов
и позволил больше сконцентрироваться на реализации правил линтера. Из подобных библиотек, выбрал ее из-за
высокого недельного количества скачиваний.
...
### Доступные команды
    ...
    * `npm run node` - проверка работоспособности в node.js (Выполнить после `npm run build`)
